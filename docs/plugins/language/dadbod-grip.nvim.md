...
结果表中按 `/` 时，输入框会把当前字段固定在 `WHERE` 后，例如光标位于 `name` 列时显示
`WHERE "name" `。只需输入该字段后面的条件，例如 `= 'Alice'`、`LIKE '%工程%'`、`> 100`
或 `IS NULL`。筛选通过 Grip 原有查询规格重新执行并回到第 1 页；重复筛选会继续使用 `AND`
叠加条件，已有排序和手工调整的列顺序会保留。Grip 原有的 `F` 仍可清除筛选。
