...
目录在 Linux 为 `~/vimwiki/`，在 Windows 为 `E:/@home/usharbisin/vimwiki/`，语法 Markdown，扩展名 `.md`；表格自动格式化和 Vimwiki 表格映射关闭，
交给 vim-table-mode。

常用上游入口是 `<Space>ww` 打开默认 Wiki 索引、Enter 跟随/创建链接、Backspace 返回；本项目
没有改这些默认映射。可用 `:lua =vim.g.vimwiki_list[1].path` 核对当前平台路径。普通 Markdown 文件
