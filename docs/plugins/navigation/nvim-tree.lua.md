...
`yy`/`dd`/`pp` 复制剪切粘贴，`yp`/`yn` 复制路径/文件名，`.` 或 `zh` 切换点文件/路径，
`I` 切换 `.gitignore` 忽略项，`R` 刷新，`?` 帮助，`q` 关闭。`s` 标记节点，`[c ]c` 跳 Git 状态，
`[d ]d` 跳诊断。

默认隐藏点文件和 `.git`，但显示 `.gitignore` 忽略的普通文件/路径；这些忽略项使用 `Comment`
高亮整体调淡，便于与正常文件区分。宽度 30，显示 Git 与诊断。`dD` 是永久文件操作，确认目标后再用。
