...
界面按 `?` 查看锁定版本的完整按键。`lazy-lock.json` 在本地锁定插件提交，已被 Git 忽略；
它不锁 Mason 工具、npm 构建产物或系统软件。首次引导失败先确认 `git` 与 GitHub 网络。

Arch Linux 与 Windows 各自维护本地锁文件。更新插件后若 Tree-sitter query 改变，还要同步
parser。上游：[lazy.nvim](https://github.com/folke/lazy.nvim)。
