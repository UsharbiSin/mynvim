...
本页基于 `main` 的 `fd118cb` 与 `windows` 的 `516a36e` 静态比较。两个分支共享主体插件和
大多数快捷键，但不能视为仅换了一条路径。
...
| Codex | Snacks terminal、命令、health、测试和文档均存在 | 当前没有 Codex 集成 |
| Snacks | 启动即加载，图片、公式、通知、终端 | 声明了 `lazy=false` 又写 filetype；无 terminal/Codex |
...
`markdown.lua`、`keymaps.lua` 和 `options.lua`。Codex 若要回迁 Windows，应整体移植模块、
health、测试、快捷键和 Snacks terminal 配置，不能只复制两个按键。
