...
Windows 会拦截 image.nvim 核心层和 Markdown 集成各自的滚动重绘，统一在停止滚动 180 毫秒后执行一次。统一刷新先删除全部 Kitty placement，再扫描当前视口并让每张可见图片只发送一次，避免 WezTerm 单图删除不完整时透明公式反复叠加、笔画变粗。Kitty 普通定位使用屏幕坐标，无法让图片直接绑定缓冲区行随文本移动，因此滚动停止后的重新定位仍是必要步骤。
