...
| 普通 | `tn` | 新标签页 |
| 普通 | `tl` / `tr` | 上一个 / 下一个标签页 |
| 普通 | `tt` | 以当前 Git 项目或文件所在目录为根节点打开或关闭文件树 |
...
在 Grip 查询结果中按 `K` 查看光标所在字段的类型和注释，按 `Ctrl-h` / `Ctrl-l` 将当前列
向左或向右移动，按 `Ctrl-s` 升序排列当前列，按 `Ctrl-d` 降序排列，按 `/` 为当前列输入
WHERE 条件筛选；光标在筛选条件行时按 `|` 切换该条件的 `AND` / `OR`，其他位置按 `|` 切换后续新增筛选的连接方式；筛选条件行上的 `d` 只删除
当前条件，数据行上的 `d` 仍删除当前行。按 `<Space>sx` 将当前结果页导出为 XLSX 或 CSV。普通代码缓冲区的
LSP `K` 不受影响。排序重新查询数据时会保留当前结果窗口的自定义列顺序。
